...
//        $link->close();
    ...
    function checkTable($name)
    {
        ...
        if ($this->link->connect_errno) {
            echo "Unable MySQL connection: " . $this->link->connect_error;
        }
...
//        $link->close();
    ...
    function addToCart($code, $tableName)
    {
        ...
        $data = $this->getGoodsInfo($code, 'goodslist');
        ...
        foreach ($data as $prod) {
            $info = $this->getGoodsInfo($code, $tableName);
            $price = $prod['price'];
            $numb = $info[0]['numb'];
            ...
            if (empty($numb)) {
                ...
                $numb++;
                if (!empty($data[0]['special_offer'])) {
                    $print_price = $this->countPrice($data[0]['special_offer'], $price, $offer_price, $offer_numb, $numb);
                } else {
                    $print_price = $this->countPrice('', $price, '', '', $numb);
                }
                $print_price = round($print_price, 2);
                $this->updateCart($code, $numb, $print_price, $tableName);
            }
        }
...
//        $link->close();
    ...
    function deleteOneProduct($code, $tableName)
    {
        ...
        $info = $this->getGoodsInfo($code, 'goodslist');
        $price = $info[0]['price'];
        $data = $this->getGoodsInfo($code, $tableName);
...
//            var_dump($numb);
            if ($numb === 0) {
                $this->deleteAllProducts($code, $tableName);
            } else {
                if (!empty($info[0]['special_offer'])) {
                    $print_price = $this->countPrice($info[0]['special_offer'], $price, $offer_price, $offer_numb, $numb);
                } else {
                    $print_price = $this->countPrice('', $price, '', '', $numb);
                }
                $print_price = round($print_price, 2);
